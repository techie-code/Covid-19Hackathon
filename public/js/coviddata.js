...
let all_time_chart, days_chart, recover_rate_chart,  death_rate_chart
...
loadSummary = async (country) => {
    ...
    await loadDeathRate(Math.floor(summary.TotalDeaths / summary.TotalConfirmed * 100))
...
loadRecoveryRate = async (rate1) => {
    // use updateSeries
    recover_rate_chart.updateSeries([rate1])
}
...
initDeathRate = async () => {
    ...
    death_rate_chart = new ApexCharts(document.querySelector('#death-rate-chart'), options)

    death_rate_chart.render()
}

loadDeathRate = async (rate) => {
    // use updateSeries
    death_rate_chart.updateSeries([rate])
}
